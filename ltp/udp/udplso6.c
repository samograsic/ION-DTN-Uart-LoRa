...
#if defined (ION_LWT)
int	udplso6(saddr a1, saddr a2, saddr a3, saddr a4, saddr a5,
...
int	main(int argc, char *argv[])
{
	...
	{
		PUTS("Usage: udplso6 {<remote engine's host name> | @}\
[:<its port number>] <remote engine ID>");
	...
	if (txbps != 0)
	{
		PUTS("NOTE: udplso6 now gets transmission data rate from \
...
compatibility, but it is ignored.");
...

	{
		...
		isprintf(memoBuf, sizeof(memoBuf),
			"[i] udplso6 is running, spec=%s, rengine=%d.",
			endpointSpec, (int) remoteEngineId);
		writeMemo(memoBuf);
	}
